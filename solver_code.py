...
class convection_diffusion_equation:
    ...
    def solver_tdma(self, number_of_grid, scheme):

        self.delta_x = self.L/(number_of_grid-1)        # end points are half-cell
        self.Pe_local = self.rho * self.u * self.delta_x / self.gamma
        ...
        Q_list[0] = self.phi_0
        P_list[0] = 0
        Q_list[number_of_grid-1] = self.phi_L
        P_list[number_of_grid-1] = 0
        phi_list[0] = self.phi_0
        phi_list[number_of_grid-1] = self.phi_L
    ...
    def plot_the_result(self,  number_of_grid, u, scheme_list):
        ...
        for scheme in scheme_list: 
            Pe_local = self.rho * self.u * delta_x / self.gamma
            start_time = time.time()
            phi_actual = self.solver_tdma(number_of_grid, scheme)
            end_time = time.time()
        ...
        self.save_fig_custom(fig, file_path='figures', file_name=f'phi_plot_grid_{number_of_grid}_u_{u}_pe_{Pe_local:.2f}', overwrite=True, dpi = 500)
        ...
        for scheme in scheme_list: 
            ...
            if mode == 'grid_size':
                ...
                for number_of_grid in number_of_grid_list:
                    x = np.linspace(0, self.L, number_of_grid)
                    delta_x = x[1] - x[0]
                ...
                def dx_to_pe(dx):
                    return self.rho * self.u * dx / self.gamma
                ...
                def u_to_pe(u):
                    return self.rho * u * delta_x / self.gamma

                def pe_to_u(pe):
                    return pe * self.gamma / (self.rho * delta_x)
